...
function instance(system, id, config) {
	...
	self.ICON_RECORD_STANDBY =
		''
	self.ICON_STOP =
		''
	...
	return [
		{
			...
			width: 12,
			label: 'Information',
			value: `<div style='margin-left: 20px;padding-left: 10px;border-left: 3px #BBBBBB solid'> 
		...
		},
		{
			...
			value:
				'When enabled the commands sent and received from Livemind Recorder will be logged.  [Default: Un-Checked]',
		},
	]
}
...
instance.prototype.initTCP = function () {
	...
	if (self.config.host) {
		...
		self.socket.on('connect', function () {
			...
			self.log(
				'info',
				'[Livemind Recorder] Connected to Livemind Recorder at IP ' + self.config.host + ' on port ' + self.config.port
			)
		})
	...
	if (data !== undefined || data !== '') {
		...
		if (data.slot) {
			...
			try {
				...
				self.log(
					'error',
					'[Livemind Recorder] Error Slot undefined. Does the "Number of Slots to Create" in module settings match the slots in the "Grid Size" in Recorder?'
				)
				self.status(
					self.STATUS_ERROR,
					'ERROR: Does the number of slots to create match the grid size in Recorder settings?'
				)
				self.setVariable('status', 'Error')
			}
		}
			...
			options: [
				{
					...
					id: 'slot',
					tooltip:
						'Select the slot to stop recording, or select All Connected Slots \r\n(Minimum selection is 1 item, add one item to remove the first item.)',
					default: [0],
					choices: self.CHOICES_SLOT,
					minSelection: 1,
				},
		...
		switch (action.action) {
			...
			case 'stopRecordingSlot':
				if (options.slot[0] === 0) {
					...
					options.slot.forEach((element) => {
						cmd +=
							'<recording_stop slot="' + element + '" uid="' + Date.now() + Math.floor(Math.random() * 100) + '" />\r\n'
					})
				}
				break
...
instance.prototype.initFeedbacks = function () {
	...
	feedbacks['slotNotReady'] = {
		...
		callback: function (feedback) {
			if (
				self.SLOTS[feedback.options.slot].source === 'none' ||
